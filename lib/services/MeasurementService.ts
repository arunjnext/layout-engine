...
export class MeasurementService {
  ...
  measureComponent(
    ...
    if (maxWidth) {
      ...
      // Ensure box-sizing matches to handle padding correctly
  ...
  private applyTemplateStyles(element: HTMLElement, config: TemplateConfig): void {
    ...
    // Apply section-specific styles (legacy support)
    const workStyles = style.spaces?.work;
    if (workStyles) {
      if (workStyles.marginTop !== undefined) {
        element.style.marginTop = `${workStyles.marginTop}px`;
      }
      if (workStyles.marginBottom !== undefined) {
